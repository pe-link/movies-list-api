...
def test_find_interval_range_max_and_min_intervals(memory_db):
    movie1 = Movie(title='Movie 1', year=2000, studios='Studio 1', winner=True)
    movie2 = Movie(title='Movie 2', year=2005, studios='Studio 2', winner=True)
    movie3 = Movie(title='Movie 3', year=2010, studios='Studio 3', winner=True)
    movie4 = Movie(title='Movie 4', year=2015, studios='Studio 4', winner=True)
    movie5 = Movie(title='Movie 5', year=2020, studios='Studio 5', winner=True)
    producer1 = Producer(name='Producer 1', movies=[movie1, movie2, movie3])
    producer2 = Producer(name='Producer 2', movies=[movie3, movie4])
    producer3 = Producer(name='Producer 3', movies=[movie4, movie5])
    ...
    # MIN INTERVALS
    assert len(win_interval_times.min_intervals) == 4
    assert sorted(win_interval_times.min_intervals, key=lambda x: x.producer_name)[0].producer_name == 'Producer 1'
    assert sorted(win_interval_times.min_intervals, key=lambda x: x.producer_name)[0].interval == 5
    assert sorted(win_interval_times.min_intervals, key=lambda x: x.producer_name)[2].producer_name == 'Producer 2'
    assert sorted(win_interval_times.min_intervals, key=lambda x: x.producer_name)[2].interval == 5

    # MAX INTERVALS
    assert len(win_interval_times.max_intervals) == 4
    assert sorted(win_interval_times.max_intervals, key=lambda x: x.producer_name)[0].producer_name == 'Producer 1'
    assert sorted(win_interval_times.max_intervals, key=lambda x: x.producer_name)[0].interval == 5
    assert sorted(win_interval_times.max_intervals, key=lambda x: x.producer_name)[2].producer_name == 'Producer 2'
    assert sorted(win_interval_times.max_intervals, key=lambda x: x.producer_name)[2].interval == 5
...
def test_mock_consecutive_wins_min_and_max_intervals():
    ...
    producer_2_mock.movies = [movie_3_mock]
    ...
    assert result.max_intervals[0].producer_name == 'Producer 1'
    assert result.max_intervals[0].interval == 5
    assert result.max_intervals[0].previous_win == 2000
    assert result.max_intervals[0].following_win == 2005
